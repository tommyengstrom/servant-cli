...
newtype instance ContextFor m (StreamBody' mods framing ctype a) = GenStreamBody {genStreamBody :: m a}
...
instance
  ...
  cliPStructWithContext_ pm _ p =
    withParamM (addBody <$> genStreamBody mx)
      <$> cliPStructWithContext_ pm (Proxy @api) p
    where
...
instance
  ...
  cliPStructWithContext_ pm _ p =
    ...
    where
      ...
      addParam = foldRequiredArgument (Proxy @mods) add (maybe id add)
      add :: a -> Request -> Request
      add = addHeader (CI.mk . T.encodeUtf8 . T.pack $ pName)
      opt :: Opt (RequiredArgument mods a)
      opt =
        Opt
...
instance (KnownSymbol desc, HasCLI m api ctx) => HasCLI m (Summary desc :> api) ctx where
  ...
  cliPStructWithContext_ pm _ =
    note [symbolVal (Proxy @desc)]
...
instance
  ...
  cliPStructWithContext_ pm _ =
    cliPStructWithContext_ pm (Proxy @subapi)
...
instance
  ...
  cliPStructWithContext_ pm _ p =
    withParamM (uncurry (&) . unAuthReq <$> genAuthReq md)
      <$> cliPStructWithContext_ pm (Proxy @api) p
    where
